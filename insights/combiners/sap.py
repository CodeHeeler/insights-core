...
@combiner(hostname, [SAPHostCtrlInstances, Lssap, HDBVersion])
class Sap(dict):
    """
    Combiner for combining the result of :class:`insights.parsers.lssap.Lssap`
    generated by command ``lssap``, 
    `insights.parsers.saphostctrl.SAPHostCtrlInstances` generated by command
    ``saphostctrl``, and `insights.parsers.sap_hdb_version.HDBVersion`
    generated by command ``HDB version``.
    ...
    def __init__(self, hostname, insts, lssap, hdbversion):
        ...
        if insts:
            ...
            for inst in hdbversion.data:
                k = inst['Instance'] # uncertain on the next few lines, using lssap above as guidance
                self.local_instances.append(k) if hn == inst['SAPLOCALHOST'] else None
                data[k] = SapInstances(k,
                                       inst['Version'])
        if not data:
